...
def handle_get_next_message(args):
    ...
    try:
        message = concurrency_manager.atomic_get_next_message(args.queue)
...
def handle_update_message(args):
    ...
    try:
        message = concurrency_manager.atomic_update_message(args.queue, args.message_id, args.content, args.status)
    except ConflictError as e:
        print(f"Error: {e}")
        return 1
